...
import TabItems from '../TabItem/index'
import AppItems from '../AppItem/index'
...
class AppStore extends Component {
  ...
  render() {
    ...
    const filterApp = this.FilteredBySearch()
    const FilteredAppList = filterApp.filter(
      ...
      <div className="bg-container">
        ...
        <ul className="tabItems">
          {tabsList.map(eachTab => (
            <TabItems
              tabsList={eachTab}
              key={eachTab.tabId}
              onClickTabId={this.onChangeTabId}
        ...
        <ul className="AppList">
          {FilteredAppList.map(eachApp => (
            <AppItems appList={eachApp} key={eachApp.appId} />
          ))}
        </ul>
      </div>
    )
